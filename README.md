...
本项目旨在围绕 **RNA-binding proteins, RBP（RNA结合蛋白）** 的结合位点数据，构建一套从 **ENCODE 数据收集、元数据整理、peak 文件筛选、原始文件下载、文件体检、主峰文件选择到标准化输出** 的可复现分析流程。

当前阶段的重点不是直接做大规模生物学结论，而是先建立一条 **稳定、清晰、可扩展的数据工程与预处理管线**，并以少量示例 RBP 为起点跑通全流程，后续再逐步扩展到更多 RBP、更多 biosample，乃至更广义的多组织体系。
...
这件事并不是简单“下载文件”这么直接，而是需要解决以下一系列问题：
...
经过以上处理，当前项目已经不再停留在“如何收集数据”的阶段，而是已经准备进入真正的生物学分析阶段。
...
1. **metadata 驱动**
   ...
   * 不是盲目下载，而是先整理元数据、后筛选文件

2. **manifest 驱动**

   * 下载行为由 manifest 控制，而不是手工逐个下载

3. **分层式结构**

   * metadata / raw / processed / results 明确分离
...
6. **先保证工程正确，再进入生物学解释**
   ...
   * 先解决路径、格式、文件选择和标准化，再做下游分析

---

## 14. 总结当前项目状态

本项目已经成功构建了一条以 ENCODE eCLIP 数据为核心、围绕示例 RBP（HNRNPK、TARDBP）和示例 biosample（HepG2、K562）展开的 **RBP 结合位点数据收集、筛选、下载、体检、主峰选择和标准化处理流程**，并已获得一批可直接用于下游注释与比较分析的标准化主 peak 文件。
